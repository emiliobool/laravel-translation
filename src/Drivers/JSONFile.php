...
class JSONFile extends Translation implements DriverInterface
{
    private $disk;
    ...
    public function saveGroupTranslations($language, $group, $translations)
    {
        ...
        $json_file = "{$this->languageFilesPath}/$language.json";
        ...
        $this->disk->put($json_file, json_encode($json, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
    }
    ...
    private function saveNamespacedGroupTranslations($language, $group, $translations)
    {
        ...
        $json_file = "$directory/$language.json";
        $json = @json_decode(file_get_contents($json_file), true);
        if (!$json) {
            $json = [];
        }
    ...
    private function saveSingleTranslations($language, $translations)
    {
        ...
        foreach ($translations as $group => $translation) {
            ...
            $json_file = "{$this->languageFilesPath}/$language.json";
            $json[$group] = $translations->get($group);
        }
        $this->disk->put(
            $json_file,
        ...
        // $this->disk->put($json_file, json_encode($json, JSON_PRETTY_PRINT));
    }
